...
if ($viewType != 'download') {
    ...
    if (!empty($result)) {
        ...
        <p>Legend - <span class="cBlue">OS: Opening Stock</span>, <span class="cRed">SA: Stock Added</span>, <span class="cGreen">SS: Stock Sale</span>, <span class="cRed">BS: Breakage Stock</span>, <span class="cBlue">CS: Closing Stock</span></p>
        ...
        <table class='table table-striped table-condensed search-table' style="color:grey">
            ...
            foreach ($productsList as $productID => $productName) {
                ...
                <tr id="<?php echo 'RowClass' . $productID; ?>" class="HideRow">
                    ...
                    <td><span class="cBlue">OS</span><br><span class="cRed">SA</span><br><span class="cGreen">SS</span><br><span class="cRed">BS</span><br><span class="cBlue">CS</span></td>
                    <?php
                    for ($i = 1; $i <= $no_of_days; $i++) {
                        foreach ($result[$i] as $row) {
                            if ($row['p']['id'] == $productID) {
                                ...
                                ?>
                                    <td onmouseover="$('#infoDiv<?php echo $productID . '-' . $i; ?>').toggle()" onmouseout="$('#infoDiv<?php echo $productID . '-' . $i; ?>').toggle()">
                                        ...
                                        <div style="position:absolute; background-color:#eee; display:none; padding:5px; border:1px solid #888; font-size:11px;" id="infoDiv<?php echo $productID . '-' . $i; ?>">
