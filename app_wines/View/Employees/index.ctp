...
<?php if ($employees) { ?>
    ...
    <table class='table' style="width:100%">
        ...
        foreach ($employees as $row) {
            ...
            <tr>
                <td><?php echo $i; ?></td>
                <td><?php echo $this->Html->link($row['Employee']['name'], ['controller' => 'employees', 'action' => 'view', $row['Employee']['id']], ['title' => 'View Employee Details - ' . $row['Employee']['name']]); ?></td>
                ...
                <td>
                    <form method="post" style="" name="remove_employee_<?php echo $row['Employee']['id']; ?>" id="remove_employee_<?php echo $row['Employee']['id']; ?>" action="<?php echo $this->Html->url("/employees/remove/" . $row['Employee']['id']); ?>">
                        <?php echo $this->Html->link('Edit', ['controller' => 'employees', 'action' => 'edit', $row['Employee']['id']], ['title' => 'Edit Employee - ' . $row['Employee']['name'], 'class' => 'small button link yellow']); ?>
                        |
