							...
							foreach($categoryInfo['CategoryProducts'] as $row) {
								...
								if(!empty($row['Images'])) {										
									foreach($row['Images'] as $image) {
										...
										$imageUrl = $this->Img->showImage('img/images/'.$imageID, array('height'=>'500','width'=>'800','type'=>'crop'), array('style'=>'', 'alt'=>$productName, 'title'=>$imageCaption, 'escape'=>false), true);
										...
										?>
										<img src="<?php echo $imageUrl;?>" data-thumb="<?php echo $imageThumbUrl;?>" alt="<?php echo $imageCaption;?>" title="#htmlcaption<?php echo $imageID;?>"/>
										...
										if($this->Session->read('Site.request_price_quote')) {
											...
											'.$this->Form->submit('Request Price Quote &raquo;', array('div'=>false, 'escape'=>false, 'style'=>'width:200px;', 'onclick'=>"showRequestPriceQuoteForm('$categoryID','$productID', '$productName')")).'
											&nbsp;|&nbsp;
											'.$this->Form->submit('Add To My Shopping List &raquo;', array('div'=>false, 'escape'=>false, 'style'=>'width:200px;', 'onclick'=>"showAddToCartForm('$categoryID','$productID', '$productName')")).'
											</div>
											';
										}
										?>
								...
								$requestWindows.='
									<div style="display:none;" id="addToShoppingListDiv'.$productID.'">'.$this->element('addtocart_form', array('productID'=>$productID, 'categoryID'=>$categoryID, 'productName'=>$productName, 'categoryName'=>$categoryName)).'</div>
									
									<div style="display:none;" id="requestPriceQuoteDiv'.$productID.'">'.$this->element('request_price_quote_form', array('productID'=>$productID, 'categoryID'=>$categoryID, 'productName'=>$productName, 'categoryName'=>$categoryName)).'</div>
								';
							?>
