...
class AppController extends Controller
{
    ...
    function parseURL()
    {
        ...
        if (isset($this->request->params['controller']) and ($this->request->params['controller'] == 'plants')) {
            $this->redirect('/');
        }
        ...
        $sitedomain = $this->Domain->findByName($dName);
        if (!empty($sitedomain)) {
            if ($sitedomain['Site']['active']) {
                ...
                foreach ($sitedomains as $row) {
                    if ($row['Domain']['default']) {
                        $defaultDomain = $row['Domain']['name'];
                        $this->Session->write('Site', $row['Site']);
                        $this->Session->write('Domain', $row['Domain']);

                        App::uses('User', 'Model');
                        $this->User = new User;

                        $siteUserID = $row['Site']['user_id'];
                        $this->User->recursive = '-1';
                        $siteUser = $this->User->findById($siteUserID);
                        $this->Session->write('Site.Account', $siteUser['User']);

                        break;
                    }
                }
                // check if the request domain is the default domain. if not then redirect to default domain.
                if ($defaultDomain) {
                    if ($dName != $defaultDomain) {
                        $redirectLink = 'http://' . $defaultDomain . $params;
                        $this->redirect($redirectLink);
                    } else {
                        if ($this->Session->read('Site.suspended') == true) {
                            if (isset($this->request->params['controller']) and ($this->request->params['controller'] == 'sites') and ($this->request->params['action'] == 'suspended')) {

                            } else {
                                $this->redirect('http://' . $dName . '/sites/suspended');
                            }
                        } else {
                            // check if site is under maintenance.
                            if (!isset($this->request->params['admin'])) {
                                if ($this->Session->read('Site.under_maintenance') == true) {
                                    if (($this->request->params['action'] == 'under_maintenance') or ($this->request->params['action'] == 'getTopNavContent') or ($this->request->params['action'] == 'login') or ($this->request->params['action'] == 'logout') or ($this->request->params['action'] == 'forgotpassword')) {

                                    } else {
                                        $this->redirect('http://' . $dName . '/sites/under_maintenance');
                                    }
                                }
                            }
                        }
