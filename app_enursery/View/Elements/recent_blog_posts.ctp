		...
		foreach($contents as $row) {
			...
			?>
				<div style="border-bottom:1px dotted #666666; margin:5px 0;">
					<p>
						<strong><?php echo $this->Html->link($blogTitle, '/blog/show/'.$blogID.'/'.$blogTitleSlug, array('title'=>$blogTitle, 'id'=>'article'.$blogID));?> </strong>
						<br>
						<span style="font-size:11px; font-style:italic;">- <?php echo $blogTime;?></span>
					</p>
					<p style="text-align: justify;" ><?php echo $blogDesc;?></p>
					<p class="more">
						<a href="<?php echo $this->Html->url('/blog/show/'.$blogID.'/'.$blogTitleSlug);?>" title="<?php echo  'Read full article: '.$blogTitle;?>">Read more</a>
					</p>	
				</div>			
			<?php
			}
