...
if (!empty($productInfo['Product']['meta_keywords'])) {
    $this->Html->meta('keywords', $productInfo['Product']['meta_keywords'], array('inline' => false));
} else {
    $this->Html->meta('keywords', $categoryName . ',' . $productName . ',buy ' . $productName . ',get price quote for ' . $productName, array('inline' => false));
}
?>
...
$this->Html->meta('description', $desc, array('inline' => false));
?>
    ...
    <div itemscope itemtype="http://schema.org/BreadcrumbList">
        <ul id="productNav">
            <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                <?php echo $this->Html->link('<span itemprop="name">' . $categoryName . '</span>', '/products/show/' . $categoryID . '/' . $categoryNameSlug, array('itemscope', 'itemtype' => 'http://schema.org/ListItem', 'escape' => false)); ?>
                <meta itemprop="position" content="1"/>
            </li>
            <li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                <?php echo $this->Html->link('<span itemprop="name">' . $productName . '</span>', '/products/details/' . $categoryID . '/' . $productID . '/' . $categoryNameSlug . '/' . $productNameSlug, array('itemscope', 'itemtype' => 'http://schema.org/ListItem', 'escape' => false)); ?>
...
<div itemscope itemtype="http://schema.org/Product">
    ...
    if (!empty($productImages)) {
        ...
        <div id="productImages" class="product-details-page-slider">
            ...
            foreach ($productImages as $row) {
                ...
                $imageUrl = $this->Html->url($this->Img->showImage('img/images/' . $imageID, array('height' => '600', 'width' => '600', 'type' => 'auto', 'quality' => '85', 'filename' => $imageCaptionSlug), array('style' => '', 'alt' => $productName, 'title' => $imageCaption), true), true);
                $imageThumbUrl = $this->Html->url($this->Img->showImage('img/images/' . $imageID, array('height' => '150', 'width' => '150', 'type' => 'crop', 'quality' => '85', 'filename' => $productNameSlug), array('style' => '', 'alt' => $productName, 'title' => $imageCaption), true), true);
            ...
            <article>
                ...
                if ($this->Session->read('Site.request_price_quote') and $showRequestPriceQuote) {
                    ...
                    <div class="floatLeft" style="margin-right:10px;">
                        <br>
                        <?php echo $this->Form->submit('Add To Cart &raquo;', array('escape' => false)); ?>
                    </div>
                    <div class='clear'></div>
    ...
    <?php
    $customMeta = '';
    $customMeta .= $this->Html->meta(array('property' => 'og:title', 'content' => $productName, 'inline' => false));
    $customMeta .= $this->Html->meta(array('property' => 'og:type', 'content' => 'product', 'inline' => false));
    $customMeta .= $this->Html->meta(array('property' => 'og:url', 'content' => $url, 'inline' => false));

    $customMeta .= ($imageUrl) ? $this->Html->meta(array('property' => 'og:image', 'content' => ($higlightImage) ? $higlightImage : $imageUrl, 'inline' => false)) : '';
    ...
    $customMeta .= $this->Html->meta(array('property' => 'og:description', 'content' => $desc, 'inline' => false));
