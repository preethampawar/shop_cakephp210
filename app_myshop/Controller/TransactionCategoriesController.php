...
class TransactionCategoriesController extends AppController
{
	...
	{
		$hideSideBar = false;
		$conditions = ['TransactionCategory.site_id' => $this->Session->read('Site.id')];
	...
	public function add()
	{
		...
		if (isset($this->request->data) and !empty($this->request->data)) {
			...
			if (!empty($data['TransactionCategory']['name'])) {
				...
				$conditions = ['TransactionCategory.name' => $data['TransactionCategory']['name'], 'TransactionCategory.site_id' => $this->Session->read('Site.id')];
			...
			if (!$error) {
				...
				$data['TransactionCategory']['site_id'] = $this->Session->read('Site.id');
	...
	public function edit($transactionCategoryId = null)
	{
		...
		if (isset($this->request->data) and !empty($this->request->data)) {
			...
			if (!empty($data['TransactionCategory']['name'])) {
				...
				//find if a similar category exists for the selected store
				$conditions = ['TransactionCategory.name' => $data['TransactionCategory']['name'], 'TransactionCategory.site_id' => $this->Session->read('Site.id'), 'TransactionCategory.id <>' => $transactionCategoryId];
				if ($this->TransactionCategory->find('first', ['conditions' => $conditions])) {
					$error = 'TransactionCategory "' . $data['TransactionCategory']['name'] . '" already exists';
				}
			} else {
	...
	public function delete($transactionCategoryId = null)
	{
		if (!$info = $this->getTransactionCategoryInfo($transactionCategoryId)) {
			...
			$conditions = ['Transaction.category_id' => $transactionCategoryId, 'Transaction.site_id' => $this->Session->read('Site.id')];
