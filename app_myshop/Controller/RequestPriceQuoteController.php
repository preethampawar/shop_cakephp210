...
class RequestPriceQuoteController extends AppController
{
	...
	function index()
	{
		...
		if ($this->request->isPost() and !empty($this->request->data)) {
			...
			} else if (!(Validation::email($data['ShoppingCart']['email']))) {
			...
			// Sanitize data
			$data['ShoppingCart']['name'] = Sanitize::paranoid($data['ShoppingCart']['name'], [' ', '-', '.']);
			$data['ShoppingCart']['phone'] = Sanitize::paranoid($data['ShoppingCart']['phone'], [' ', '+']);
...
';
					...
					$this->Session->setFlash('Thank you for your request.', 'default', ['class' => 'success']);
	...
	public function admin_index()
	{
		...
		$conditions = ['ShoppingCart.site_id' => $this->Session->read('Site.id'), 'request_price_quote' => '1'];
		$this->paginate = [
	...
	public function admin_delete($shoppingCartID = null)
	{
		if ($this->request->isPost()) {
			App::uses('ShoppingCart', 'Model');
			$this->ShoppingCart = new ShoppingCart;
			$conditions = ['ShoppingCart.id' => $shoppingCartID, 'ShoppingCart.site_id' => $this->Session->read('Site.id')];
			$cartInfo = $this->ShoppingCart->find('first', ['conditions' => $conditions]);

			if (empty($cartInfo)) {
				$this->Session->setFlash('Information unavailable', 'default', ['class' => 'error']);
			} else {
				// delete shopping cart products
				$conditions = ['ShoppingCartProduct.shopping_cart_id' => $shoppingCartID];
				...
				$this->Session->setFlash('Price quote has been removed', 'default', ['class' => 'success']);
			}
		} else {
