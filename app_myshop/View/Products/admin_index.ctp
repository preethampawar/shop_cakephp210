					...
					foreach ($products as $row) {
						...
						<tr>
							...
							<td>
								<?php
								if ($productActive) {
									echo $this->Html->link($this->Html->image('round_button_green.jpg', ['alt' => 'active', 'title' => 'Click to deactivate', 'width' => '16']), '/admin/products/setInactive/' . $productID, ['style' => 'color:green;', 'escape' => false], 'Are you sure you want to deactivate this product? Deactivating will hide this product from public.');
								} else {
								...
								<?php
								echo $this->Html->link($productName, '/admin/products/edit/' . $productID, ['title' => $productName]);
								?>
							</td>
							<td>
							...
							<td style="text-align:center;">
								<?php
								if ($productFeatured) {
									echo $this->Html->link($this->Html->image('test-pass-icon.png', ['alt' => 'active', 'title' => 'Click to set this product as featured', 'width' => '16']), '/admin/products/unsetFeatured/' . $productID, ['style' => 'color:green;', 'escape' => false], 'Are you sure you want to remove this product from featured product list?');
								} else {
									echo $this->Html->link($this->Html->image('test-fail-icon.png', ['alt' => 'active', 'title' => 'Click to remove this product from featured product list', 'height' => '12', 'width' => '12']), '/admin/products/setFeatured/' . $productID, ['escape' => false, 'style' => 'color:red; margin:2px;'], 'Are you sure you want to set this product as featured? Doing so, will show this product in featured list.');
								}
								?>
							</td>
							<td style="text-align:center;">
								<?php echo $this->Html->link('Edit', '/admin/products/edit/' . $productID, ['title' => 'Edit Product: ' . $productName]); ?>
								|
								<?php echo $this->Html->link('Images', '/admin/images/manageProductImages/' . $productID, ['title' => 'Mange Product Images: ' . $productName]); ?>
								|
								<?php echo $this->Html->link($this->Html->image('error.png', ['alt' => 'active', 'title' => 'Click to remove this product']), '/admin/products/deleteProduct/' . $productID, ['escape' => false, 'style' => 'color:red;', 'title' => 'Delete Product: ' . $productName], 'This product may belong to different categories and will be deleted from all. Are you sure you want to delete this product - ' . $productName . '?'); ?>
							</td>
						</tr>
						<?php
					}
