...
$this->Html->meta('keywords', 'products gallery, photo gallery, all products', ['inline' => false]);
$this->Html->meta('description', 'View all products belonging to ' . $this->Session->read('Site.title') . ' at one place. Showing products photo gallery from ' . $this->Session->read('Site.title'), ['inline' => false]);
?>
						...
						foreach ($categoryInfo['CategoryProducts'] as $row) {
							...
							if (!empty($row['Images'])) {
								foreach ($row['Images'] as $image) {
									...
									$imageUrl = $this->Img->showImage('img/images/' . $imageID, ['height' => '600', 'width' => '600', 'type' => 'auto', 'quality' => '75', 'filename' => $productNameSlug], ['style' => '', 'alt' => $productName, 'title' => $imageCaption, 'escape' => false], true);
									...
									<div style="float:left; border:0px solid #fff; width:auto; padding:2px;">
										<a href="<?php echo $imageUrl; ?>"
										   title='<?php echo $categoryLink . ' &raquo; ' . $productLink . '<p>' . $imageCaption . '</p>'; ?>'>
											<?php
											echo $this->Img->showImage('img/images/' . $imageID, ['height' => '60', 'width' => '60', 'type' => 'crop', 'quality' => '50', 'filename' => $productNameSlug], ['style' => '', 'alt' => $productName, 'title' => $imageCaption]);
											?>
										</a>
									</div>
