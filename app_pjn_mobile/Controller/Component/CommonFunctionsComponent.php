...
class CommonFunctionsComponent extends Component
{
	...
	public function getFranchiseInfo($franchiseId = null)
	{
		...
		if (!$franchiseId) {
			...
			$conditions = ['Franchise.id' => $franchiseId, 'Franchise.is_deleted' => 0, 'Franchise.store_id' => $this->Session->read('Store.id')];
			if ($franchiseInfo = $this->Franchise->find('first', ['conditions' => $conditions])) {
