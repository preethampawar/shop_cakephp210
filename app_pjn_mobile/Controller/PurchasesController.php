...
class PurchasesController extends AppController
{
	...
	public function selectCategory($invoiceId)
	{
		App::uses('Invoice', 'Model');
		$invoice = new Invoice();
		$invoiceInfo = $invoice->findById($invoiceId);
		...
		$categoriesList = $productCategory->find('list', ['conditions' => ['ProductCategory.store_id' => $this->Session->read('Store.id')]]);

		$this->set(compact('categoriesList', 'invoiceInfo'));
	}

	public function addProduct($invoiceId, $categoryId)
	{
		$error = null;
		...
		$conditions = ['Product.store_id' => $this->Session->read('Store.id'), 'Product.product_category_id' => $categoryId];


		$productsList = [];
		$productsInfo = $productModel->find('all', ['conditions' => $conditions, 'order' => 'Product.name', 'recursive' => '-1']);
		...
		if ($this->request->isPost() or $this->request->isPut()) {
			...
			if (!$error) {
				$productModel->bindModel(['belongsTo' => ['ProductCategory']]);

				if (!$productInfo = $productModel->findById($data['Purchase']['product_id'])) {
			...
			if (!$error) {
				...
				if ($this->Purchase->save($data)) {
					...
					$msg = $productInfo['Product']['name'] . ' successfully added to invoice';
					$this->successMsg($msg);

					$this->redirect('/purchases/addProduct/' . $invoiceId . '/' . $categoryId);
				}
			}
		} else {
//			$data['Purchase']['category_id'] = $categoryId;
//			$this->data = $data;
		}
		...
		$invoiceProducts = $this->Purchase->find('all', ['conditions' => $conditions, 'order' => 'Purchase.created ASC', 'recursive' => '0']);
		...
		App::uses('ProductCategory', 'Model');
		$productCategory = new ProductCategory();
		$categoryInfo = $productCategory->findById($categoryId);
		$this->set(compact('productsInfo', 'productsList', 'invoiceProducts', 'invoiceInfo', 'categoryInfo'));
	}
	...
	public function add()
	{
		...
		$productsInfo = $productModel->find('all', ['conditions' => $conditions, 'order' => 'Product.name', 'recursive' => '-1']);
		$productsList = $productModel->find('list', ['conditions' => $conditions, 'order' => 'Product.name', 'recursive' => '-1']);
	...
	private function checkValidCsvData($fileInfo)
	{
		...
		$productModel = new Product();
		$productModel->bindModel(['belongsTo' => ['ProductCategory']]);
		$storeProducts = $productModel->find('all', ['conditions' => ['Product.store_id' => $this->Session->read('Store.id')], 'fields' => ['Product.id', 'Product.name', 'Product.box_qty', 'Product.box_buying_price', 'ProductCategory.name', 'ProductCategory.id']]);
