...
class InvoicesController extends AppController
{
	...
	public function add($invoiceType = null)
	{
		$errorMsg = null;
						...
						if ($data['Invoice']['name'] = trim($data['Invoice']['name'])) {
							...
							if ($this->Invoice->find('first', ['conditions' => $conditions])) {
								...
								if ($this->Invoice->save($data)) {
									...
									$this->redirect('/invoices/selectInvoice/' . $invoiceInfo['Invoice']['id']);
								} else {
		...
		$this->set(compact('suppliersList', 'invoiceTypes', 'franchiseList', 'invoiceType'));
	}
					...
					if (!$errorMsg) {
						if (!empty($data['Invoice']['invoice_date'])) {
							...
							if ($this->Invoice->find('first', ['conditions' => $conditions])) {
								...
								$data['Invoice']['store_id'] = $this->Session->read('Store.id');
								$data['Invoice']['supplier_name'] = isset($data['Invoice']['supplier_id']) && !empty($data['Invoice']['supplier_id']) ? $suppliersList[$data['Invoice']['supplier_id']] : '';
								...
								if ($this->Invoice->save($data)) {
									if ($invoiceInfo['Invoice']['invoice_type'] == 'sale') {
										...
										$this->successMsg($msg);
										$this->redirect('/sales/selectCategory/' . $invoiceID);
									} else {
										...
										$msg = 'Invoice updated successfully';
										$this->successMsg($msg);
										$this->redirect('/purchases/selectCategory/' . $invoiceID);
									}
