...
class SalesController extends AppController
{
	...
	public function addProduct($invoiceId, $categoryId)
	{
		...
		$invoiceInfo = $this->Invoice->findById($invoiceId);
		...
		if ($this->request->isPost() or $this->request->isPut()) {
			...
			if (!$error) {
				...
				if ($this->Sale->save($data)) {
					...
					$this->successMsg($msg);
					$this->redirect('/sales/addProduct/' . $invoiceId . '/' . $categoryId);
				}
			}
		}
		...
		$this->set(compact('productsInfo', 'productsList', 'invoiceInfo', 'invoiceProducts', 'categoryInfo'));
	}
	...
	public function removeProduct($saleId = null)
	{
		if ($this->request->isPost()) {
			if ($saleInfo = $this->CommonFunctions->getSaleInfo($saleId)) {
				...
				$this->successMsg('"' . $saleInfo['Sale']['product_name'] . '" removed from the list');
			} else {
