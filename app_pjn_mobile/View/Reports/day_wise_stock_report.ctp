...
if ($viewType != 'download') {
	...
	<?php if ($showForm) { ?>
		...
		<div id="paramsDiv">
			...
			<div style="float:left; clear:none; margin-top:5px;">
				<br>
				&nbsp;&nbsp;<?php echo $this->Form->submit('Get Report', ['id' => 'SubmitForm', 'title' => '', 'type' => 'submit', 'onclick' => 'return submitButtonMsg()', 'div' => false]); ?>
			</div>
	...
	if (isset($result)) {
		if (!empty($result)) {
			?>
			<h3>Product Stock Report: From '<?php echo date('d M Y', strtotime($fromDate)); ?>' to
				'<?php echo date('d M Y', strtotime($toDate)); ?>'</h3>
